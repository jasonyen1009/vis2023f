...
FileAttachment("artist4.csv").csv()
)}
...
md`## 結論
### 從上圖中，我們可以看出：
- ### 北部地區： 高達12.1% 的人選擇減少使用包裝材料，顯示該地區對環保和減少浪費的關注程度較高，可能受到相應的宣傳和教育活動的影響。

- ### 中部地區： 低至3.12% 的人選擇減少包裝材料使用，可能反映中部地區對該議題的關注程度相對較低，或者缺乏普遍意識。

- ### 南部地區： 約5.07% 的人選擇減少使用包裝材料，介於北部和中部之間，顯示南部地區對環保有一定的意識，但仍有提升的空間。

- ### 東部地區： 最低的比例為2.34%，可能表示東部地區對減少包裝材料的關注度相對較低，或者缺乏相應的環保教育和宣傳活動。`
)}
...
d3
  .scaleOrdinal()
  .domain(["北部", "中部", "南部", "東部", "國外","工作室", "替代空間", "美術館", "減少包裝材及文宣印製", "使用無毒媒材、再生材料、廢物利用素材等", "工作場所、活動展場的節約能源"])
  //.range(d3.schemePaired)
  .range(["#241468","#9F0D7F","#EA1179","#F79BD3","#610C9F","#940B92","#940B92"])
  .unknown("#BEBEBE")
)}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["artist4.csv", {url: new URL("./artist4.csv", import.meta.url), mimeType: "text/csv", toString}]
  ]);
